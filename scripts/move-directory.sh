#!/usr/bin/env bash
# script for moving the devxp directory and copying over the .files

old_path="/usr/local/bin/devxp-files"
new_path="$HOME/.local/devxp-files"

# Ensure that the directory exists
mkdir -p "$new_path"

# Clone the repository to the new directory
git clone -b move https://github.com/elhub/devxp-linux.git "$new_path/devxp-linux"
git config --global --add safe.directory "$new_path/devxp-linux"

# create the .files in the new location?

# Move all contents of the old directory to the new directory, excluding devxp-linux
rsync -av --progress "$old_path/" "$new_path" --exclude devxp-linux

############################################ rc file modifications ############################################
script_to_trigger="/usr/local/bin/devxp-files/devxp-linux/scripts/rc-notifications.sh"
new_notification_path="$HOME/.local/devxp-files/devxp-linux/scripts/rc-notifications.sh"

# Function to update the rc files if they exist
add_or_edit_line() {
    local file="$1"
    local line_to_add="$2"
    local found=false

    # Check if the line already exists in the file
    if grep -Fxq "$line_to_add" "$file"; then
        echo "Updating in $file. With new devxp-files location."
        # Append the line to the file
        sed -i "s/$script_to_trigger/$new_notification_path/g" "$file"
        found=true
    fi
}

# Bash
if [ -n "$BASH_VERSION" ]; then
    add_or_edit_line "$HOME/.bashrc" "$script_to_trigger"
fi

# Zsh
if command -v zsh &> /dev/null; then
    add_or_edit_line "$HOME/.zshrc" "$script_to_trigger"
fi

# Fish
if command -v fish &> /dev/null; then
    add_or_edit_line "$HOME/.config/fish/config.fish" "$script_to_trigger"
fi

# Ksh
if [ -n "$KSH_VERSION" ]; then
    add_or_edit_line "$HOME/.kshrc" "$script_to_trigger"
fi

# Dash
if [ -n "$DASH_VERSION" ]; then
    add_or_edit_line "$HOME/.dashrc" "$script_to_trigger"
fi

# Csh
if [ -n "$CSH_VERSION" ]; then
    add_or_edit_line "$HOME/.cshrc" "$script_to_trigger"
fi

# Tcsh
if [ -n "$TCSH_VERSION" ]; then
    add_or_edit_line "$HOME/.tcshrc" "$script_to_trigger"
fi

# PowerShell
if [ -n "$PSVersionTable" ]; then
    add_or_edit_line "$HOME/.bash_profile" "$script_to_trigger"
fi

# Elvish
if command -v elvish &> /dev/null; then
    add_or_edit_line "$HOME/.elvish/rc.elv" "$script_to_trigger"
fi

############################################ rc file modifications ############################################

# Delete the old directory
$new_path/devxp-linux/delete-directory.sh
