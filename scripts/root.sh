...
if ! grep -q "/usr/local/bin/devxp-linux/scripts/reminder.sh" /etc/anacrontab; then
    # If the line doesn't exist, append it to /etc/anacrontab
    echo "${YELLOW}Configured anacron job to run /usr/local/bin/devxp-linux/scripts/reminder.sh once a week${NC}"
    echo "${YELLOW}This can be edited in the /etc/anacrontab file${NC}"
    cat << EOF >> /etc/anacrontab
@weekly 15      cron.weekly             /usr/local/bin/devxp-linux/scripts/reminder.sh
EOF
else
